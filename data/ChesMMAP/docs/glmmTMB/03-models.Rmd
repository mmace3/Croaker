...
kbl(glmmTMB_fall_table, 
    ...
    col.names = c("Model", "AIC", "%DE", "Mean", "Dispersion", "ZI"),
    #format = "latex",
    caption = "Summary of models for ChesMMAP fall standardized abundance. An NA for the AIC value indicates the model did not converge or the hessian was not invertible.") %>%
  kable_styling(latex_options = c("striped", "hold_position", 
                                  "scale_down"),
                font_size = 7) %>%
  # landscape() %>%
...
All of the models combined all ages and used a negative binomial distribution. Although models with a year and region interaction are included below we decided to just focus on models with an additive effect for year and almost all models with a year and region interaction had convergence issues.
